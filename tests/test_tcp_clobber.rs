...
#[tokio::test]
async fn single_thread_limited_rate_and_total() -> TestResult {
...
#[tokio::test]
async fn multi_thread_limited_rate_and_total() -> TestResult {
    let addr = echo_server();
    let config = ConfigBuilder::new(addr)
        .rate(Some(10))
        .limit(Some(20))
...
#[tokio::test]
async fn rateless_with_duration() -> TestResult {
    let addr = echo_server();
    let config = ConfigBuilder::new(addr)
        .workers(4)
        .threads(Some(2))
...
#[tokio::test]
async fn with_fuzz_config() -> TestResult {
    let addr = echo_server();
    let config = ConfigBuilder::new(addr)
        .workers(1)
        .threads(Some(1))
